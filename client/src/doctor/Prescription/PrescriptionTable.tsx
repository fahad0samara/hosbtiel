...
const Prescription = () => {
 ...
 useEffect(() => {
  ...
  const fetchData = async () => {
   try {
    const response = await fetch(`http://localhost:3000/doctor/Prescription/${Doctor._id}`)
 ...
 if (error) {
  return <p className="text-center font-medium text-red-600">Error: {error.message}</p>
 }
    ...
    <div className="text-xl  font-medium  ml-20">
     ...
     <h1>
      ...
      <span className="text-cyan-300 font-medium ml-2">
       {Doctor.user.createdAt.slice(0, 10).split('-').reverse().join('-')}
      </span>
     </h1>
    </div>
    <h1 className=" "></h1>
      ...
      <table className="w-full text-sm text-left  ">
       <thead className="bg-gray-200">
        <tr className=" uppercase text-sm leading-normal">
         <th className="py-2 px-4 font-medium text-gray-700">Patient Name</th>
         <th className="py-2 px-4 font-medium text-gray-700">Date of Prescription</th>

         <th className="py-2 px-4 font-medium text-gray-700">medication</th>
       ...
       <tbody>
        {prescriptions.map((prescription: any) => (
         <tr key={prescription._id} className="hover:bg-gray-100 border-b border-gray-200 py-10">
          <td className="py-2 px-4">
           {prescription.patient.name.firstName} {prescription.patient.name.LastName}
          </td>
          <td className="py-2 px-4">{prescription.createdAt.slice(0, 10).split('-').reverse().join('-')}</td>
