...
const MyCalendar = () => {
 ...
 const workingHoursEvents = Doctor.availableDaysAndHours.map((dayAndHour) => {
  return {
   start: moment(`${dayAndHour.day} ${dayAndHour.startTime}`, 'dddd HH:mm A').toDate(),
   end: moment(`${dayAndHour.day} ${dayAndHour.endTime}`, 'dddd HH:mm A').toDate(),
   ...
   try {
    ...
    const appointmentEvents = appointments.map((appointment) => {
     return {
      ...
      end: moment(`${appointment.appointmentDate} ${appointment.appointmentTime}`, 'YYYY-MM-DD HH:mm')
       .add(1, 'hours')
       .toDate(),
