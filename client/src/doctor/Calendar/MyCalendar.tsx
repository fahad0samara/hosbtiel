...
const MyCalendar = () => {
 ...
 useEffect(() => {
  const getAppointments = async () => {
   setLoading(true)
   try {
    const response = await axios.get(`http://localhost:3000/doctor/appointments/${Doctor._id}`, {
     headers: {
  ...
  >
   <div className="ml-28 p-6">
    <h1 className={`${dark ? 'text-white' : 'text-black'} text-xl`}>
     Welcome <span className="font-bold text-cyan-300">{Doctor.name.firstName}</span> to your calendar
    </h1>
    <p
     className="
            italic
