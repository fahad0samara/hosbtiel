...
const Stats = () => {
 ...
 useEffect(() => {
  ...
  const getData = async () => {
   try {
    const res = await axios.get(`http://localhost:3000/doctor/stats/${Doctor._id}`)
 ...
 return (
  <div className={'grid lg:grid-cols-4  text-center order-last md:order-first mt-20 md:mt-0'}>
   {' '}
   <div>
   ...
   <div>
    {' '}
    <p className="font-bold  text-xl">
     {prescriptions ? loading ? 'Loading...' : <span>{prescriptions}</span> : <span>0</span>}
    </p>{' '}
   ...
   <div>
    {' '}
    <p className="font-bold  text-xl">
     {appointment ? loading ? 'Loading...' : <span className="    text-lg">{appointment}</span> : <span>0</span>}
    </p>{' '}
