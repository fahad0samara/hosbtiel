     ...
     {loading ? (
      ...
      >
       ...
       {doctors &&
        doctors.map((doctor) => {
         return (
          <tbody key={doctor._id} className=" text-sm font-extrabold ">
           <tr className="border-b border-gray-200 hover:bg-gray-100 hover:text-black">
              ...
              <div className="mr-2">
               {
                //loading the image
                doctor.avatar ? (
                 <img className="w-8 h-8 rounded-full" src={doctor.avatar} alt="" />
                ) : (
                 <div>
                  <h1>loading...</h1>
                 </div>
            ...
            <td className="py-3ml-2 text-center">
             <div className="flex items-center justify-center">
              <span className="ml-2">
               {
                //createdAt
                doctor.user.createdAt.toString().substring(0, 10).split('-').reverse().join('-')
               }
              </span>
             </div>
            </td>
            ...
            <td className="py-3  text-center">
             <div className="flex items-center justify-center">
              <span>{doctor.date.toString().substring(0, 10).split('-').reverse().join('-')}</span>
             </div>
            </td>
            <td className="py-3 px-6 text-center">
             <span
              className={`py-1 px-2 rounded-full text-white text-md  text-center border-b border-gray-100 hover:bg-black hover:text-white ${
               doctor.specialty === 'Cardiologist' ? 'bg-green-400' : ''
                              ...
                              ${doctor.specialty === 'Oncologist' ? 'bg-cyan-500' : ''}
                              ${doctor.specialty === 'Pediatrician' ? 'bg-rose-400' : ''}
                              ${doctor.specialty === 'Dermatologist' ? 'bg-red-600' : ''}${
               doctor.specialty === 'Endocrinologist' ? 'bg-purple-500' : ''
              }
                              ${doctor.specialty === 'Gastroenterologist' ? 'bg-pink-300' : ''}
               ...
               doctor.specialty === 'Urologist' ? 'bg-teal-300 ' : ''
                               ...
                               ${doctor.specialty === 'Gynecologist' ? 'bg-yellow-500' : ''}`}
             >
              {doctor.specialty}
             </span>
            </td>
