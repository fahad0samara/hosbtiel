...
const Table = () => {
 ...
 useEffect(() => {
  ...
  const fetchData = async () => {
   try {
    const response = await fetch(`http://localhost:3000/admin/doctor/Prescription/${id}`)
 ...
 const downloadPrescription = async (prescriptionId) => {
  ...
  try {
   const res = await axios.get(`http://localhost:3000/admin/doctor/prescriptions/download/${prescriptionId}/${id}`, {
  ...
  <div>
   {loading ? (
      ...
      <tbody>
       {prescriptions.map((prescription) => (
        <tr key={prescription._id}>
         <td data-label="PATIENT NAME">{prescription.patient.name.firstName}</td>
    ...
    <div className="md:-mt-24 flex flex-col justify-center items-center">
     <h1 className="text-2xl font-bold text-center my-3 text-cyan-300">Prescription History</h1>
     <p className="text-gray-500 text-center ml-11">The doctor does not have any Prescription</p>
    </div>
   )}
  </div>
 )
