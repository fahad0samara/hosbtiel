...
const MyCalendar = () => {
 ...
 useEffect(() => {
  const getAppointments = async () => {
   setLoading(true)
   try {
    const response = await axios.get(`http://localhost:3000/user/all-appointments/${Patient._id}`)
    ...
    const sortedAppointments = appointmentEvents.sort((a, b) => (a.start > b.start ? 1 : -1))
 ...
 const eventPropGetter = (event, start, end, isSelected) => {
  let backgroundColor = ''
  const colors = ['red', 'green', 'purple', 'bronze', 'black', 'blue', 'orange', 'pink', 'yellow', 'gray']
  ...
  >
   <div className="ml-28 p-6">
    <h1 className={`${dark ? 'text-white' : 'text-black'} text-xl`}>
     Welcome <span className="font-bold text-cyan-300">{Patient.name.firstName}</span> to your calendar
    </h1>
    <p
     className="
            italic
    ...
    {loading ? (
     ...
     <p
      className="
            ...
            "
     >
      You have <span className="font-bold text-cyan-300">{events.length}</span> appointments
     </p>
    )}
   </div>
   ...
   <div className={`${dark ? 'bg-black' : 'bg-gray-100'} flex justify-between items-center ml-28`}></div>
