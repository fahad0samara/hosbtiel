...
const AppointmentForm = () => {
 ...
 useEffect(() => {
  if (doctorAvailability && appointmentDate) {
   const selectedDay = doctorAvailability.find((day) => day.day === appointmentDate)
   if (selectedDay) {
 ...
 useEffect(() => {
  ...
  const fetchDoctors = async () => {
   try {
    ...
    const selectedDoctorData = futureAvailability.find((doctor) => doctor._id === selectedDoctor)
    if (selectedDoctorData) {
     setDoctorAvailability(selectedDoctorData.availableDaysAndHours)
    } else {
     setDoctorAvailability([])
 ...
 const handleSubmit = async (e) => {
  ...
  const appointmentDateObject = moment.utc(appointmentDate, 'dddd, MMMM D YYYY').toDate()
  ...
  try {
   ...
   console.log('Error while creating appointment: ', error.response.data.error)
   setLoading(false)
  }
 }
 return (
  ...
  >
    ...
    {errorMessage && (
     ...
     >
      <div className=" rounded-lg p-6">
       <p className="text-lg font-medium mb-4">There was an error while creating your appointment.</p>
   ...
   <div className=" md:ml-20 p-6  rounded-lg ml-20">
    ...
    <ul className="list-disc list-inside mb-4">
     <li className="mb-2">Please arrive 15 minutes early to check in and fill out any necessary paperwork.</li>
     <li className="mb-2">
      If you have any questions or concerns about your appointment, please call our office at
      <a href="tel:[phone number]" className="text-cyan-600 hover:text-cyan-800 transition-colors">
       [phone number]
      </a>
     ...
     <li className="mb-2">
      If you have any insurance information or documents that need to be submitted before your appointment, please bring
      them with you.
     </li>
    </ul>
   </div>
    ...
    >
     <div className="">
      ...
      <form onSubmit={handleSubmit}>
          ...
          {doctors &&
           ...
           doctors.map((doctor) => (
            <option key={doctor._id} value={doctor._id}>
             {doctor.name.firstName} {doctor.name.lastName} ({doctor.specialty})
            </option>
           ))}
         </select>
        </label>
       ...
       <label htmlFor="symptoms" className=" text-xl font-semibold px-1 mr-28 mt-5">
        Symptoms:
       </label>
       <div className="symptoms-container" style={{ height: '200px', overflowY: 'scroll' }}>
        <div className="grid grid-cols-3 gap-4 px-3  mb-5  p-4">
         <div className="">
          <input className="mr-1" type="checkbox" value="headache" onChange={handleChange} />
          Headache
         </div>
         <div>
          <input className="mr-1" type="checkbox" value="fever" onChange={handleChange} />
          Fever
         </div>
         <div>
          <input className="mr-1" type="checkbox" value="cough" onChange={handleChange} />
          Cough
         </div>
         <div>
          <input className="mr-1" type="checkbox" value="body pain" onChange={handleChange} />
          Body Pain
         </div>
         <div>
          <input className="mr-1" type="checkbox" value="sore throat" onChange={handleChange} />
          Sore Throat
         </div>
         <div>
          <input className="mr-1" type="checkbox" value="fatigue" onChange={handleChange} />
          Fatigue
         </div>
         <div>
          <input className="mr-1" type="checkbox" value="stomach ache" onChange={handleChange} />
          Stomach Ache
         </div>
         <div>
          <input className="mr-1" type="checkbox" value="vomiting" onChange={handleChange} />
          Vomiting
         </div>
         <div>
          <input className="mr-1" type="checkbox" value="diarrhea" onChange={handleChange} />
          Diarrhea
         </div>
         <div>
          <input type="checkbox" value="nausea" onChange={handleChange} />
          Nausea
         </div>
         <div>
          <input className="mr-1" type="checkbox" value="dizziness" onChange={handleChange} />
          Dizziness
         </div>
         <div>
          <input className="mr-1" type="checkbox" value="chest pain" onChange={handleChange} />
          Chest Pain
         </div>
         <div>
          <input className="mr-1" type="checkbox" value="shortness of breath" onChange={handleChange} />
          Shortness of Breath
         </div>
         <div>
          <input className="mr-1" type="checkbox" value="loss of appetite" onChange={handleChange} />
          Loss of Appetite
         </div>
         <div>
          <input className="mr-1" type="checkbox" value="loss of taste" onChange={handleChange} />
          Loss of Taste
         </div>
         <div>
          <input className="mr-1" type="checkbox" value="loss of smell" onChange={handleChange} />
          Loss of Smell
         </div>
         <div>
          <input className="mr-1" type="checkbox" value="muscle pain" onChange={handleChange} />
          Muscle Pain
         </div>
         <div>
          <input className="mr-1" type="checkbox" value="skin rash" onChange={handleChange} />
          Skin Rash
         </div>
        </div>
        <div>
       ...
       <div className=" text-sm font-bold my-4">Selected Symptoms: {symptoms.join(', ')}</div>

       <div className="flex -mx-3">
        <div className="w-full px-3 mb-5">
         <button className="bg-cyan-300 text-white px-4 py-2 rounded font-medium w-full" type="submit">
          Book Appointment
         </button>
        </div>
       </div>
