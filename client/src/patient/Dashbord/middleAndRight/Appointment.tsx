 ...
 useEffect(() => {
  ...
  const intervalId = setInterval(() => {
   ...
   if (timeDiff < 0) {
    ...
    const localAppointmentTime = appointmentDateTime.local().format('h:mm A')
    setLoading(true)
    setTimeUntilNextAppointment(`Appointment at ${localAppointmentTime} (${timeString} remaining)`)
   }
   setLoading(false)
  }, 1000)
  setLoading(false)
        ...
        <div className="flex flex-row justify-between items-center md:ml-7 ml-4">
         {timeUntilNextAppointment && timeUntilNextAppointment ? (
          <div className="flex flex-row ">
           ...
           <p className=" text-sm ml-2">{timeUntilNextAppointment}</p>
          </div>
         ) : null}
        </div>
       </div>
...
lg:w-60
     ...
     {appointmentData.currentAppointment ? (
      <div className="grid grid-rows-3 gap-4  w-[21rem]   ">
       ...
       >
        ...
        >
         ...
         <p className="text-sm italic mr-12 text-gray-400">{appointmentData.currentAppointment.doctor.specialty}</p>
        </div>
        ...
        {showModal && (
         <AppointmentModal appointmentData={appointmentData.currentAppointment} onClose={handleCloseModal} />
        )}
       </div>
       ...
       >
        <div className="grid grid-cols-2 gap-5  p-4">
         <div className="flex flex-row justify-center items-center space-x-1">
          ...
          <h1 className="text-sm font-bold ml-2">{appointmentData.currentAppointment.appointmentTime}</h1>
         </div>
         ...
         <div className="flex flex-row justify-center items-center space-x-2 ">
          ...
          <h1 className="text-sm font-bold ">{formatDate(appointmentData.currentAppointment.appointmentDate)}</h1>
         </div>
        </div>
       </div>
      </div>
      ...
      >
       <div className="flex flex-col justify-center items-center ">
        <h1 className="text-lg font-bold text-cyan-300 mt-4">You have no upcoming appointment,</h1>
        <h1 className="text-md italic  mt-4 ">please add an appointment If you have any</h1>
       ...
       <Link
        ...
        className="bg-cyan-300 rounded-full w-full
                ...
                "
       >
        <h1 className="flex justify-center items-center mt-1">Add Appointment</h1>
       </Link>
      </div>
     )}
    </div>
       ...
       <div className="grid grid-rows-3 gap-4  w-[21rem]   ">
        ...
        >
         ...
         >
          ...
          <p className="text-sm italic mr-12 text-gray-400">{appointmentData.nextAppointment.doctor.specialty}</p>
         </div>
        </div>
        ...
        >
         <div className="grid grid-cols-2 gap-5  p-4">
          <div className="flex flex-row justify-center items-center space-x-1">
           ...
           <h1 className="text-sm font-bold ml-2">{appointmentData.nextAppointment.appointmentTime}</h1>
          </div>
          ...
          <div className="flex flex-row justify-center items-center space-x-2 ">
           ...
           <h1 className="text-sm font-bold ">{formatDate(appointmentData.nextAppointment.appointmentDate)}</h1>
          </div>
         </div>
        </div>
       </div>
