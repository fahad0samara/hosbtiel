...
const Stats = () => {
 ...
 useEffect(() => {
  ...
  const getData = async () => {
   try {
    const res = await axios.get(`http://localhost:3000/user/stats/${Patient._id}`)
  ...
  <div className="bgc ">
   ...
   <div className="grid grid-cols-2 gap-3">
    ...
    <div className=" flex flex-col justify-center items-center p-1">
     ...
     <h2>{prescriptions ? loading ? 'Loading...' : <span>{prescriptions}</span> : <span>0</span>}</h2>
    </div>

    <div className=" flex flex-col justify-center items-center p-1">
     <h1 className="font-bo italic text-center mb-2">event</h1>
     <h2>{events ? loading ? 'Loading...' : <span>{events}</span> : <span>0</span>}</h2>
    </div>
   </div>
  </div>
 )
