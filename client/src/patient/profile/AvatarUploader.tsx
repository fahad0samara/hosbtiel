...
function AvatarUploader() {
 ...
 async function handleUpload() {
  ...
  try {
   setIsUploading(true)
   const response = await fetch(`http://localhost:3000/user/avatar/${Patient._id}`, {
    method: 'POST',
    body: formData,
   })
    ...
    <div className="w-48 h-48 bg-gray-100 mx-auto rounded-full shadow-2xl overflow-hidden">
     {
      ...
      !imageLoaded || !Patient.avatar ? (
       ...
       <img
        src={selectedFile ? URL.createObjectURL(selectedFile) : Patient.avatar}
        alt="avatar"
